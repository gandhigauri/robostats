...
class wma(object):
	...
	def WeightedMajorityAlgorithm(self, N, eta, T, nature):
		...
		for t in range(T):
			...
			weather = random.choice(['sunny','windy'])
			x = self.receive_obs(N, t, weather) #receive expert advices
			#print "obs" + str(x) + "\n"
			summation = 0
			for n in range(N):
				summation = summation + (x[n] * w[n])
			#print "summation" + str(summation) + "\n"
			y_est = self.sign(summation) #learner predicts outcome
			#print "y_est" + str(y_est) + "\n"
			y_true = self.receive_truth(nature, t, w, x, weather)  #nature sends truth
	...
	def receive_obs(self, N, t, weather):
		obs_vector = [0] * N
		trial_num = t + 1
		obs_vector[0] = 1 #optimistic
		obs_vector[1] = -1 #pessimistic
		if (trial_num % 2) is 0: #expert depending on trial numbers
			obs_vector[2] = 1
		else:
			obs_vector[2] = -1
		if weather is 'sunny': #expert depending on weather
			obs_vector[3] = 1
		else:
			obs_vector[3] = -1
	...
	def plot_losses(self, learner_loss, expert_losses):
		...
		plt.plot([expert[0] for expert in expert_losses],'r-',
			...
			[expert[3] for expert in expert_losses],'m-',
			learner_loss,'y-')
		plt.xlabel('t')
		plt.ylabel('losses')
